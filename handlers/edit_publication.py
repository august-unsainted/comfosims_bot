...
from utils.keyboards import get_back_kb, generate_edition_kb
from utils.publication_utils import select_publication, create_admin_notification, prepare_admin_message, get_link, continue_form
...
async def edit_publication(callback: CallbackQuery):
    ...
    text = config.texts.get('edit').format(pub['title'])
    kb = config.edit_keyboard(f'{table}_{pub_id}', 'edit')
    await config.handle_message(callback, {'text':         text,
                                               'reply_markup': kb})
...
async def edit_data(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text=config.texts.get(field),
...
async def set_data(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(config.texts.get('send'),
...
async def confirm_delete(callback: CallbackQuery):
    table, pub_id = callback.data.split('_')[:2]
    kb = config.edit_keyboard(f'{table}_{pub_id}', 'confirm_delete')
    await config.handle_message(callback,
